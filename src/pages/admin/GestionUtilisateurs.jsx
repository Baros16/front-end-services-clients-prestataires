...
import {PageHeader,SearchInput,AlertBanner,EmptyState,DataTable,RoleTag,StatusBadge,RatingStars,} from '../../components/commons';
...
const filtered = users.filter(u => {const q = search.toLowerCase();
const matchSearch =
...
<div className="w-9 h-9 rounded-full bg-sl-100 flex items-center justify-center
font-bold text-[13px] text-sl-700 shrink-0">
{row.avatarInitial}
</div>
<div>
<p className="m-0 text-[14px] font-semibold text-sl-900">
{row.fullName}
</p>
<p className="m-0 text-[12px] text-sl-400">
{row.email}
</p>
</div>
</div>
...
header: 'TÉLÉPHONE',
render: (row) => (
<span className="text-[13px] text-sl-600">
{row.phone}
</span>
),
},
...
header: 'MISSIONS',
render: (row) => (
<span className="text-[14px] font-bold text-sl-900">
{row.missionsCount}
</span>
),
},
...
render: (row) => (
<StatusBadge
variant={row.status === 'active' ? 'actif' : 'suspendu'}
size="sm"
/>
),
},
...
{/* Tableau */}
<div className="bg-white rounded-[var(--radius-lg)] border border-sl-200
