...
export default function TraitementLitigeSC() {
  const [litige] = useState(mockLitigeData.litige);
  const [parties] = useState(mockLitigeData.parties);
  const [loading] = useState(false);
  const [error] = useState(null);

  const [status, setStatus] = useState("ouvert");
  const [activeParty, setActiveParty] = useState("client");
  const [clientMessages, setClientMessages] = useState(mockLitigeData.clientMessages);
  const [providerMessages, setProviderMessages] = useState(mockLitigeData.providerMessages);
  ...
  const handleCloseLitige = async () => {
    const confirmed = window.confirm("Voulez-vous vraiment clôturer ce litige ? Cette action est definitive.");
    if (!confirmed) return;
    try {
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle={`${litige.motif.title} · ${formatMontantComplet(litige.originalQuote.total)} sequestres`}
        actions={
          <StatusBadge
