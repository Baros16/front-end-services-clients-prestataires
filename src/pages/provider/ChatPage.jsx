...
import { ClientPanel }     from '../../components/provider/chat/ClientPanel';
...
export default function ChatPage() {
  ...
  const {
    ...
    dismissToast,
    deleteMessage,
  } = useChat(activeId, 'provider');

  // ── Contexte mission + client (panel droit) ──────────────────────────────
  ...
  useEffect(() => {
    ...
    getProviderConversationContext(activeId)
      .then(setContext)
      .catch(console.error)
  ...
  const handleAppel = () => {
    if (context?.client?.phone) {
      window.location.href = `tel:${context.client.phone}`;
    }
  };
  ...
  // ── Titre du header ──────────────────────────────────────────────────────
  const headerTitle = headerLoading || !context?.client ? (
    ...
    <div className="flex items-center gap-2 min-w-0">
      <Avatar
        initial={context.client.avatarInitial}
        size="sm"
        isOnline={context.client.isOnline}
      />
        ...
        <div className="flex-1 flex flex-col min-h-0 overflow-hidden"
          ...
          >
            {messagesLoading ? (
              ...
              messages.map(msg => {
                const isOwn = msg.senderRole === "provider";
                return (
                  <MessageBubble
                    key={msg.id}
                    message={msg}
